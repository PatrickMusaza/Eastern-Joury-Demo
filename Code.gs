...
function saveItem(item) {
  const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(ITEM_DATA_SHEET);
...
function clearItemsForBill(billNo) {
  ...
  const rowsToDelete = data.map((row, index) => row[3] === billNo ? index + 2 : null).filter(index => index !== null); // Find rows with matching Bill No
